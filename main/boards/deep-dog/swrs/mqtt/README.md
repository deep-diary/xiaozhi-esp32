...
联调默认 broker `broker.emqx.io:1883`（公共站匿名；局域网可选 `192.168.31.25:1883`，NVS 可覆盖）。**未绑定**时 `device_id` 默认为 **`dev`**（Topic `deepdiary/deep-dog/dev/`、RTSP path `deep-dog/dev`）；**已绑定**后切换为 STA MAC 紧凑串（如 `aabbccddeeff`）。NVS 显式写入 `device_id` 可覆盖（仅联调）。绑定/解绑见 [00-pairing](./modules/00-pairing.md)。  
Hub / ingest 走 `wss://broker.emqx.io:8084/mqtt`，与设备公共站为同一 broker（TCP 1883 与 WSS 8084 互通）。MQTTX 可直连 `broker.emqx.io:1883` 或 `wss://broker.emqx.io:8084/mqtt`。

### MQTTX / 脚本验收清单

**网页同款路径（推荐）**：`wss://broker.emqx.io:8084/mqtt`  
...
1. 连接公共站 TCP `broker.emqx.io:1883`（当前设备默认；或局域网 `192.168.31.25:1883` / 外网 WSS 对齐前端）。
