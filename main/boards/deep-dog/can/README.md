...
- **唯一通信链路**：机器狗使用**一路 CAN** 连接 12 个电机，不再使用 UART 底盘。本模块在板级初始化时启动（波特率与 `config.h` 中 `CAN_TX_GPIO` / `CAN_RX_GPIO` 一致）。
- **使用者**：`../motor/` 通过本模块收发电机指令与反馈；`dog` / `leg` 不直接访问 CAN，只调用 motor 层。

## 被使用的模块

本模块被以下模块使用：
- **电机模块** (`../motor/`): 电机控制指令收发
- **云台模块** (`../gimbal/`): 云台控制指令（其他板型）
