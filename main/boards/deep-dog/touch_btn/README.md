...
| **长按 2**（无组合窗口时） | `startContinuousForward()` **持续前进**，直至短按 **2+3** 同时停走 |
| **长按 3**（无组合窗口时） | `startContinuousBackward()` **持续后退**，直至短按 **2+3** 停走 |
| **短按 2 + 短按 3**（两键曾同时按下，且**均未长按**） | `stopContinuousLocomotion()`：停止持续行走，**保持当前关节姿态**（不再发新目标） |
| **长按 1 后的组合窗口内长按 2 / 3** | 仍为 **`goForwardSteps(5)` / `goBackSteps(5)`**（与「持续跑」区分） |

### 「长按 1 → 短按 2 / 3」如何区分短按与长按

- **短按**：按下后在 **未触发该键长按** 前释放 → 在 **释放** 时判定为短按（避免与「长按 2/3」冲突）。
- **长按 2 / 3**：由 `TouchButtonController` 在按住足够周期后上报 `kLongPress`；**无组合窗口**时进入持续前/后退；**有组合窗口**时仍为连发 5 步。
