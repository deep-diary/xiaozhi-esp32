...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    if (show_mqtt_broker_config_) {
        nvs_handle_t mqtt_nvs;
        if (nvs_open("deep_dog_mqtt", NVS_READONLY, &mqtt_nvs) == ESP_OK) {
            char host[128] = {0};
            size_t host_size = sizeof(host);
            if (nvs_get_str(mqtt_nvs, "broker_host", host, &host_size) == ESP_OK) {
                mqtt_broker_host_ = host;
            }
            int32_t port = 0;
            if (nvs_get_i32(mqtt_nvs, "broker_port", &port) == ESP_OK && port > 0) {
...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            cJSON_AddBoolToObject(json, "show_mqtt_broker_config", this_->show_mqtt_broker_config_);
            if (this_->show_mqtt_broker_config_) {
                if (!this_->mqtt_broker_host_.empty()) {
                    cJSON_AddStringToObject(json, "broker_host", this_->mqtt_broker_host_.c_str());
                }
                cJSON_AddNumberToObject(json, "broker_port", this_->mqtt_broker_port_);
            }
    ...
    httpd_uri_t advanced_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // 保存 deep-dog 板级 MQTT broker（NVS deep_dog_mqtt）
            if (this_->show_mqtt_broker_config_) {
                nvs_handle_t mqtt_nvs;
                esp_err_t mqtt_err = nvs_open("deep_dog_mqtt", NVS_READWRITE, &mqtt_nvs);
                if (mqtt_err == ESP_OK) {
                    cJSON *broker_host = cJSON_GetObjectItem(json, "broker_host");
                    if (cJSON_IsString(broker_host) && broker_host->valuestring) {
                        this_->mqtt_broker_host_ = broker_host->valuestring;
                        mqtt_err = nvs_set_str(mqtt_nvs, "broker_host", this_->mqtt_broker_host_.c_str());
                        if (mqtt_err != ESP_OK) {
                            ESP_LOGE(TAG, "Failed to save broker_host: %d", mqtt_err);
                        }
                    }
                    cJSON *broker_port = cJSON_GetObjectItem(json, "broker_port");
                    if (cJSON_IsNumber(broker_port)) {
                        this_->mqtt_broker_port_ = broker_port->valueint;
                        if (this_->mqtt_broker_port_ <= 0) {
                            this_->mqtt_broker_port_ = 1883;
                        }
                        mqtt_err = nvs_set_i32(mqtt_nvs, "broker_port", this_->mqtt_broker_port_);
                        if (mqtt_err != ESP_OK) {
                            ESP_LOGE(TAG, "Failed to save broker_port: %d", mqtt_err);
                        }
                    }
                    mqtt_err = nvs_commit(mqtt_nvs);
                    if (mqtt_err != ESP_OK) {
                        ESP_LOGE(TAG, "Failed to commit deep_dog_mqtt NVS: %d", mqtt_err);
                    }
                    nvs_close(mqtt_nvs);
                } else {
                    ESP_LOGE(TAG, "Failed to open deep_dog_mqtt NVS: %d", mqtt_err);
                }
            }
            ...
            ESP_LOGI(TAG, "Saved settings: ota_url=%s, max_tx_power=%d, remember_bssid=%d, sleep_mode=%d, broker=%s:%d",
                this_->ota_url_.c_str(), this_->max_tx_power_, this_->remember_bssid_, this_->sleep_mode_,
                this_->mqtt_broker_host_.c_str(), this_->mqtt_broker_port_);
            return ESP_OK;
        },
        .user_ctx = this
    };
